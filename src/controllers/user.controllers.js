...
const logoutUser = asyncHandler(async (req, res) => {
  ...
  await User.findByIdAndUpdate(
    ...
    {
      new: true, // so we can return new information
    }
  );
  console.log(user);
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    throw new ApiError(
      500,
      "Something goes wrong while refreshing access token"
    );
  }
});
...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    {
      $match: {
        // Don't use like this -->  _id : req.user?._id  BCZ it is in string format and
        //in aggregration pipeline we need to give it in
        // Explicitly designed Mongoose Object ID
        // Use this -- >
